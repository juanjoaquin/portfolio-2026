...
export function Sidebar({
  ...
  pages,
  style,
  allowDossierStyle = true,
  onStyleChange,
  onPageSelect,
}: SidebarProps) {
  const activeItemRef = useRef<HTMLLIElement>(null);
  const displayStyle = allowDossierStyle ? style : "simple";

  useEffect(() => {
    if (displayStyle !== "dossier" || !open) return;
    activeItemRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" });
  }, [currentPage, displayStyle, open]);

  return (
    <aside
      className={`hidden h-full min-h-0 shrink-0 flex-col overflow-hidden bg-sidebar-bg text-sidebar-fg transition-all duration-300 lg:flex ${
    ...
    >
      ...
      <nav
        ...
        aria-label={displayStyle === "dossier" ? "Miniaturas del dossier" : "Navegación del dossier"}
      >
        <ul className={`flex flex-col ${displayStyle === "dossier" ? "gap-2" : "gap-0.5"}`}>
          {pages.map((page) => {
